  ...
  <main class="admin-content">
    ...
    <div class="glass-card" style="padding: 1.5rem; overflow-x: auto;">
      <table class="data-table">
        ...
        <tbody>
          <?php if (empty($orders)): ?>
            <tr><td colspan="7" style="text-align: center; color: var(--text-muted); padding: 3rem;">No orders match the filter criteria.</td></tr>
          <?php else: ?>
            <?php foreach ($orders as $order): ?>
              <tr>
                ...
                <td>
                  <select class="form-input" style="padding: 0.25rem 0.5rem; font-size: 0.8rem; width: auto;" onchange="updateOrderStatusAdmin('<?= $order['order_id'] ?>', this.value)">
                ...
                <td style="text-align: right;">
                  <a href="<?= ROOT_PATH ?>/order-detail.php?id=<?= $order['order_id'] ?>" class="btn btn-secondary btn-sm" title="View Order & QR">
